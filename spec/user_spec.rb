describe User do
  ...
  describe 'user authentication' do
    before { @user = User.create(name: 'Raoul', email: 'diraulo@makersacademy.se', password: 'password', password_confirmation: 'password') }

    it 'succeeds with valid credentials' do
      expect(User.authenticate('diraulo@makersacademy.se', 'password')).to eq @user
    end

    it 'fails with invalid credentials' do
      expect(User.authenticate('diraulo@makersacademy.se', 'wrong-password')).to eq nil
    end
  end
end
